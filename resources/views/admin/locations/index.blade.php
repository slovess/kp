...
<div style="margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
    ...
    <div style="color: #666; font-size: 14px;">
        Всего локаций: <strong>{{ $locations->total() }}</strong>
    </div>
</div>

<a href="{{ route('admin.locations.create') }}" class="add-button">➕ Добавить локацию</a>
...
<table>
    ...
    <tbody>
        @forelse($locations as $location)
            <tr>
                <td data-label="Номер">{{ $location->id }}</td>
                <td data-label="Название">{{ $location->title }}</td>
                <td data-label="Товаров">{{ $location->goods()->count() }}</td>
                <td data-label="Создана">{{ $location->created_at->format('d.m.Y') }}</td>
                <td data-label="Действия">
                    <a href="{{ route('admin.locations.edit', $location->id) }}" title="Редактировать">✏️</a>
                    <form action="{{ route('admin.locations.destroy', $location->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" title="Удалить" onclick="return confirm('Удалить локацию?')">🗑️</button>
                    </form>
                </td>
            </tr>
        @empty
            <tr><td colspan="5" style="text-align:center;">Нет локаций</td></tr>
        @endforelse
    </tbody>
</table>
