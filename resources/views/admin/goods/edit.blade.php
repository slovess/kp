...
<form action="{{ route('admin.goods.update', $good->id) }}" method="POST" enctype="multipart/form-data" style="max-width: 500px;">
    @csrf
    @method('PUT')
    ...
    <div>
        <label><input type="checkbox" name="isPopular" {{ old('isPopular', $good->isPopular) ? 'checked' : '' }}> Популярный</label>
    </div>

    <div>
        <label><input type="checkbox" name="isPresence" {{ old('isPresence', $good->isPresence) ? 'checked' : '' }}> В наличии</label>
    </div>
